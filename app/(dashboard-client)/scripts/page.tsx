...
import { ArrowLeft, Plus, FileText, Save, Trash2, Tag, Edit, Search, RefreshCw, PlusCircle } from "lucide-react"
...
import { messageScriptsService, type MessageScript, type MessageBlock } from "@/lib/message-scripts-service"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function ScriptsPage() {
  ...
  const [messageBlocks, setMessageBlocks] = useState<MessageBlock[]>([{ id: "1", content: "", delay: 2 }])
  ...
  const addMessageBlock = () => {
    setMessageBlocks((prev) => [...prev, { id: Date.now().toString(), content: "", delay: 2 }])
  }
  ...
  const updateMessageBlockContent = (id: string, content: string) => {
    setMessageBlocks((prev) => prev.map((block) => (block.id === id ? { ...block, content } : block)))
  }

  // Atualizar o delay de um bloco de mensagem
  const updateMessageBlockDelay = (id: string, delay: number) => {
    setMessageBlocks((prev) => prev.map((block) => (block.id === id ? { ...block, delay } : block)))
  }
  ...
  const handleSaveScript = async () => {
    ...
    if (activeTab === "advanced" && !messageBlocks.some((block) => block.content.trim())) {
      toast({
        title: "Erro",
    ...
    try {
      ...
      const mainContent = activeTab === "simple" ? scriptContent : messageBlocks[0]?.content || ""

      const blocksToSave = activeTab === "advanced" ? messageBlocks : [{ id: "1", content: scriptContent, delay: 0 }]

      if (editingScript) {
        // Atualizar script existente
        const updatedScript = await messageScriptsService.updateScript(editingScript.id, {
          name: scriptName,
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 container py-8 px-6">
        <div className="flex flex-col gap-8 max-w-4xl mx-auto">
          <div className="flex items-center justify-between">
            <h2 className="text-3xl font-bold tracking-tight">Scripts de Mensagem</h2>
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="text-lg font-medium mb-2">
                  {searchTerm ? "Nenhum script encontrado" : "Nenhum script de mensagem"}
                </h3>
              ...
              {filteredScripts.map((script) => (
                <Card key={script.id} className="overflow-hidden">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => openEditScriptDialog(script)}>
                    ...
                    {script.description && (
                      <CardDescription className="mt-1 line-clamp-2">{script.description}</CardDescription>
                    )}
                  </CardHeader>
                  <CardContent>
                  ...
                  <CardFooter className="flex justify-between py-3 text-xs text-muted-foreground">
                    <span>Criado em {new Date(script.createdAt).toLocaleDateString()}</span>
                    <Button
                      variant="ghost"
                      size="sm"
      ...
      <Dialog open={showNewScriptDialog} onOpenChange={setShowNewScriptDialog}>
        <DialogContent className="sm:max-w-md md:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editingScript ? "Editar Script" : "Novo Script de Mensagem"}</DialogTitle>
            <DialogDescription>
              {editingScript
              ...
              <TabsContent value="simple" className="space-y-4 mt-2">
                <div className="grid gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Use {"{{variavel}}"} para inserir dados dinâmicos como nome, pedido, etc.
                  </p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="advanced" className="space-y-4 mt-2">
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <Button variant="outline" size="sm" onClick={addMessageBlock}>
                  ...
                  <ScrollArea className="h-[350px] pr-4">
                    <div className="space-y-4">
                      {messageBlocks.map((block, index) => (
                        <div key={block.id} className="border rounded-md p-4 space-y-3">
                          <div className="flex justify-between items-center">
                          ...
                          <Textarea
                            value={block.content}
                            onChange={(e) => updateMessageBlockContent(block.id, e.target.value)}
                          ...
                          {index < messageBlocks.length - 1 && (
                            <div className="flex items-center gap-2">
                              <Label htmlFor={`delay-${block.id}`} className="whitespace-nowrap">
                              ...
                              <Input
                                ...
                                value={block.delay}
                                onChange={(e) =>
                                  updateMessageBlockDelay(block.id, Number.parseInt(e.target.value) || 0)
                                }
                                className="w-20"
                              />
                  ...
                  <p className="text-xs text-muted-foreground">
                    Use {"{{variavel}}"} para inserir dados dinâmicos. Configure o delay entre mensagens.
                  </p>
                </div>
              </TabsContent>
            </Tabs>
